...
protocol SheetsViewControllerProtocol: AnyObject {
    func dismissSheets()
}

import UIKit

class SheetsViewController: UIViewController, SheetsViewControllerProtocol {
    ...
    private lazy var stackView: UIStackView = {
        let sv = UIStackView(arrangedSubviews: [
            makeActionButton(icon: "person.2.badge.plus.fill", title: "Create a Group", subtitle: "Add a new group", action: #selector(didTapGroup)),
            makeActionButton(icon:"camera.fill", title: "Capture Expense", subtitle: "Take a picture of your receipt", action: #selector(didTapPicture)),
            makeActionButton(icon:"square.and.pencil", title: "Add Expenses", subtitle: "Add your receipt manually", action: #selector(didTapManual)),
        ])
        sv.axis = .vertical
        sv.spacing = 16
        return sv
...
//    }
    private func makeActionButton(icon: String, title: String, subtitle: String, action: Selector) -> UIButton {
        ...
        attributedTitle.append(AttributedString("\(title)\n", attributes: AttributeContainer([
            .font: UIFont(name: "SFPro-Semibold", size: 16) ?? .systemFont(ofSize: 16, weight: .semibold),
            .foregroundColor: titleColor
        ])))
        attributedTitle.append(AttributedString(subtitle, attributes: AttributeContainer([
...
//        config.imagePadding = 14
        ...
        config.contentInsets = NSDirectionalEdgeInsets(top: 14, leading: 16, bottom: 14, trailing: 10)
...
//        button.imageView?.contentMode = .scaleAspectFit
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        view.backgroundColor = UIColor(red: 0.057, green: 0.129, blue: 0.236, alpha: 1)
        setupLayout()
    ...
    private func setupLayout() {
        ...
        [titleLabel, closeButton, stackView].forEach { $0.translatesAutoresizingMaskIntoConstraints = false; container.addSubview($0) }
...
struct SheetsViewPreview: UIViewControllerRepresentable {
    func makeUIViewController(context: Context) -> SheetsViewController {
        return SheetsViewController()
    }
    func updateUIViewController(_ uiViewController: SheetsViewController, context: Context) {}
}
